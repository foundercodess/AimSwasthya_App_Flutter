// patient_section/p_view_model/auth_view_model.dart
import 'dart:async';
import 'dart:convert';
import 'package:aim_swasthya/patient_section/p_repo/auth_repo.dart';
...
class PatientAuthViewModel extends ChangeNotifier {
  ...
  // Navigation type management moved to UserRoleViewModel
  // int _navType = 1;
  ...
  Future<void> patientRegisterApi(
    ...
    try {
      ...
      _patientAuthRepo.patientRegisterApi(data).then((value) async {
        Utils.show(value['message'], context);
        if (value['status'] == true) {
          final patientId = value['data']['patient']['patient_id'];
          ...
          await notificationViewModel.sendProfileCompletionNotification(
            patientId: patientId,
            doctorId: 0, // No doctor involved in registration
            role: 'patient',
            context: context,
            isCompleted: true,
          );
