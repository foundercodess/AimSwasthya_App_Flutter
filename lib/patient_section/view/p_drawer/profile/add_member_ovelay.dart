// patient_section/view/p_drawer/profile/add_member_ovelay.dart
...
class _AddMemberOverlayState extends State<AddMemberOverlay> {
  ...
  Future<void> _saveFamilyMember(BuildContext context) async {
    ...
    final members = vm.patientHomeModel?.data?.familyMembers;

    if (vm.selectedMemberIndex != -1 &&
        members != null &&
        members.length > vm.selectedMemberIndex!) {
      familyMemberId =
          members[vm.selectedMemberIndex!].familyMemberId?.toString();
    }
    ...
    if (upsertVM.upsertFamilyMemberModel?.status == true) {
      ...
      vm.setSelectedMemberIndex(-1);

      await vm.patientHomeApi(context);
