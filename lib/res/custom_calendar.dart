...
extension DateTimeExt on DateTime {
  ...
  DateTime addMonth(int count) {
    ...
    return DateTime(newYear, adjustedMonth, adjustedDay);
  }
...
class _HeaderState extends State<_Header> {
        ...
        Row(
          children: [
            GestureDetector(
              ...
              onTap: () => widget.onChange(widget.selectedMonth.addMonth(-1)),
            ),
            TextConst(
          ...
          children: [
            GestureDetector(
              child: const SizedBox(
                  ...
                  height: 25,
                  child: Icon(Icons.arrow_back_ios, size: 13)),
              onTap: () => widget.onChange(widget.selectedMonth.addMonth(-12)),
            ),
            Text(
  ...
  void _showYearPicker() {
    // getSearchYear.clear();
    // for (int i = 0; i < 100; i++) {
    //   final monthsBack = 12 * i;
    //   final yearDate = widget.selectedMonth.addMonth(-monthsBack);
    //   getSearchYear.add({'months': monthsBack, 'year': yearDate.year});
    // }
    ...
    for (int i = 0; i < 51; i++) {
      ...
      if (uniqueYears.add(year)) {
        getSearchYear.add({'months': (currentYear - year) * 12, 'year': year});
      }
    }
    showDialog(
      context: context,
            ...
            children: [
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final year = getSearchYear[index]['year'];
                    print(year);
                    final isSelected = year == widget.selectedMonth.year;
                    return ListTile(
                      title: TextConst(
                        year.toString(),
                        size: Sizes.fontSizeSix,
